...
import { listConjunctions, optimizeManeuver, levelOf } from '../api/orbitguard'
...
export default function ManeuversPage() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const cList = await listConjunctions()
        if (Array.isArray(cList)) {
          // Fetch candidate info for each conjunction
          const enriched = await Promise.all(
            cList.map(async (c) => {
              const candidates = await optimizeManeuver(c.id)
              return { ...c, maneuverCandidates: candidates || [] }
            })
          )
          setConjunctions(enriched)
        }
      } catch (e) {
        console.warn('Error loading maneuvers page data:', e)
      } finally {
        setLoading(false)
      }
    }
  ...
  return (
    <div className="space-y-5 p-5">
      ...
      {loading ? (
        <div className="text-xs text-ink-faint animate-pulse">Loading maneuver candidates...</div>
      ) : (
        <>
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
            {eligible.map((c) => {
              const meta = riskLevelMeta[levelOf(c)] || riskLevelMeta.green
              const best = (c.maneuverCandidates || []).find((m) => m.status === 'SAFE' || m.recommended)
              return (
                <div key={c.id} className="panel p-4">
                  ...
                  <div className="grid grid-cols-2 gap-y-1 text-xs">
                    ...
                    <span className="text-right font-mono text-risk-green">
                      {best ? `${(best.deltaV || best.dv_ms || 0).toFixed(2)} m/s` : 'None yet'}
                    </span>
                  </div>
                  <Link
                    to={`/maneuver/${c.id}`}
