...
def candidate_schema(candidate: Dict[str, Any], safety_target: float,
    ...
    pc_after = candidate.get("pc_after")
    resolved = pc_after is not None and not math.isnan(pc_after) and pc_after < safety_target
    return ManeuverCandidateSchema(
        ...
        primary_threat_resolved=bool(resolved),
